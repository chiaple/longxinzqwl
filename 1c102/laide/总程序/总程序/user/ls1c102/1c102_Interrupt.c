...
int receive_cnt=0;
uint8_t receive_buf1[MAX_receive]; // 串口1接收缓冲区
uint8_t receive_index1 = 0;        // 串口1接收索引
uint8_t receive_flag1 = 0;         // 串口1接收标志

extern uint32_t step;
extern int dir;
extern int count;
extern int count_step;
unsigned char BYJ_Code_8_ccw[8] = {0x09, 0x01, 0x03, 0x02, 0x06, 0x04, 0x0c, 0x08}; // 逆时针8拍
unsigned char BYJ_Code_8_cw[8]  = {0x08, 0x0c, 0x04, 0x06, 0x02, 0x03, 0x01, 0x09}; // 顺时针8拍
unsigned char BYJ_Code_4_ccw[4] = {0xE, 0xD, 0xB, 0x7}; // 逆时针4拍
unsigned char BYJ_Code_4_cw[4]  = {0xE, 0x7, 0xB, 0xD}; // 顺时针4拍
unsigned char index = 0;
volatile int motor_done = 0; // 定义一个标志位，表示电机是否完成
    ...
    if(IntReg  & IRQ_TIMER)//Timer
    {
        if(TIM_GetITStatus(TIM_FLAG_Trigger))
        {
            if (step == 8)
            {
                if (dir == 0)
                {
                    BYJ_gpio_write(BYJ_Code_8_cw[index]); // 顺时针
                }
                else if (dir == 1)
                {
                    BYJ_gpio_write(BYJ_Code_8_ccw[index]); // 逆时针
                }
                else if (dir == 2)
                {
                    BYJ_gpio_stop(); // 停止
            ...
            else if (step == 4)
            {
                if (dir == 0)
                {
                    BYJ_gpio_write(BYJ_Code_4_cw[index]); // 顺时针
                }
                else if (dir == 1)
                {
                    BYJ_gpio_write(BYJ_Code_4_ccw[index]); // 逆时针
                }
                else if (dir == 2)
                {
                    BYJ_gpio_stop(); // 停止
